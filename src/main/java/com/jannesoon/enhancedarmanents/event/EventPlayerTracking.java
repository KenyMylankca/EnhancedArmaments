...
import com.jannesoon.enhancedarmanents.capabilities.IEnemyLevel;
import com.jannesoon.enhancedarmanents.leveling.EnemyLevel;
import com.jannesoon.enhancedarmanents.network.PacketEnemyLevel;
import com.jannesoon.enhancedarmanents.util.EnumAttributeModifierOperations;
			...
			if (!event.getTarget().world.isRemote)
			{
				...
				if (mob != null)
				{
					...
					if (enemyLevel != null)
					{
						...
						{	
							EnhancedArmanents.network.sendToAll(new PacketEnemyLevel(enemyLevel.getEnemyLevel(), mob.getEntityId()));
							setAttributeModifiers(mob, level);
						}
						else
						{
							int newLevel = EnemyLevel.getRandomLevel(event.getEntity().world.rand).ordinal();
	...
	private void setAttributeModifiers(EntityMob mob, int level)
	{
		if (level == 1)
		{
			if (mob.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).getModifier(UUID.fromString("e8da868e-5a50-4376-87b9-35701f9f937a")) == null && mob.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getModifier(UUID.fromString("5d17b4c2-2043-45a3-bbb8-b7da6b054a23")) == null)
			{
				...
				//EnhancedArmanents.LOGGER.info(mob + " mob health " + mob.getHealth() + " / " + mob.getMaxHealth());
			}
		}
